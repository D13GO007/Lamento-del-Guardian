Shader "UHFPS/EyeBlink"
{
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
        ...
        float2 scaledUV = ClampAndScaleUVForBilinearPostProcessTexture(uv);
        int2 positionSS = int2(scaledUV * (_ScreenSize.xy / _RTHandleScale.xy));

        float4 color = LOAD_TEXTURE2D_X(_InputTexture, positionSS);

        float aspectRatio = _VignetteAspectRatio + 1.0 / (1.0 - _Blink) - 1.0;
        float2 vd = float2(uv.x - 0.5, (uv.y - 0.5) * aspectRatio);
