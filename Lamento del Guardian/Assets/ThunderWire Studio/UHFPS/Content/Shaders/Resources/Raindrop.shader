Shader "UHFPS/Raindrop"
{
    ...
    half4 CustomPostProcess(Varyings input) : SV_Target
    {
        ...
        float2 maskUV = uv * _Tiling;

        float2 raindropGravity = float2(0.0, _DropletsGravity);
        float2 raindropPan = _Time.y * raindropGravity + maskUV;
        ...
        float2 distortedUV = uv + (rain.xy / (_ScreenSize.xy / _RTHandleScale.xy));
        distortedUV = ClampAndScaleUVForBilinearPostProcessTexture(distortedUV);
