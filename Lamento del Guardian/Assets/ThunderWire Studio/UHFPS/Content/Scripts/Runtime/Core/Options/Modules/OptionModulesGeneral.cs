...
namespace UHFPS.Runtime
{
    ...
    public class OptionDisplayMonitor : OptionModule
    {
        ...
        public override void OnApplyOption()
        {
            ...
            if (value < 0 || value >= _displayLayout.Count)
                return;

            if (IsChanged)
            {
                var targetDisplay = _displayLayout[value];
        ...
        public override void OnLoadOption(bool fromFile)
        {
            ...
            int display = _displayLayout.FindIndex(d => d.name == main.name);
            Behaviour.SetOptionValue(display >= 0 ? display : 0);
        }
        ...
        {
            Screen.GetDisplayLayout(_displayLayout);
            GString[] gStrings = _displayLayout
                .ConvertAll(d => new GString(d.name))
                .ToArray();
