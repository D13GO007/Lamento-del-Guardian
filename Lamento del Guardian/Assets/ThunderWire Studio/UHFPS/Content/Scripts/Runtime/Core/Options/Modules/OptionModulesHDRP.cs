        ...
        public GString[] QualityLevels =
{
    ...
    [Serializable]
    // 0 - Off, 1 - FSR2, 2 - DLSS
    public class UpscalingMode : OptionModule
    {
        public override string ContextName => "HDRP/Upscaling Mode";

        public int DefaultValue = 0;
        public GString[] UpscalingModes =
        {
            new("*Off", ""),
            new("*FSR2", ""),
            new("*DLSS", ""),
        ...
        {
            int mode = (int)Value;
            if (IsChanged) SetUpscalingMode(mode);
            Options.SerializableData[Name] = new(mode);
        }
        ...
        public override void OnLoadOption(bool fromFile)
        {
            if (fromFile && CheckOption(JTokenType.Integer, out int value))
            {
                SetUpscalingMode(value);
                Behaviour.SetOptionValue(value);
                return;
            }
            
            SetUpscalingMode(DefaultValue); // Default Off
            Behaviour.SetOptionValue(DefaultValue);
        }

        private void SetUpscalingMode(int mode)
        {
            Options.CameraData.allowDynamicResolution = mode > 0;
            if (mode > 0)
            {
                Options.CameraData.allowDeepLearningSuperSampling = mode == 2;
                Options.CameraData.allowFidelityFX2SuperResolution = mode == 1;
            }
        }

        public override void OnBuildOption(OptionBehaviour behaviour)
        {
            behaviour.SetOptionData(new StorableCollection() { { "options", UpscalingModes } });
        }
    }
    
    [Serializable]
    // 0 - Quality, 1 - Balanced, 2 - Performance, 3 - Ultra Performance
    public class UpscalingQuality : OptionModule
    {
        public override string ContextName => "HDRP/Upscaling Quality";

        public int DefaultValue = 1;
        public GString[] UpscalingQualities =
        {
            new("*Quality", ""),
            new("*Balanced", ""),
            new("*Performance", ""),
        ...
        {
            int quality = (int)Value;
            if (IsChanged || Options.CameraData.allowDynamicResolution) 
                SetUpscalingQuality(quality);
            Options.SerializableData[Name] = new(quality);
        }
        ...
        public override void OnLoadOption(bool fromFile)
        {
            if (fromFile && CheckOption(JTokenType.Integer, out int value))
            {
                SetUpscalingQuality(value);
                Behaviour.SetOptionValue(value);
                return;
            }
            
            SetUpscalingQuality(DefaultValue); // Default Balanced
            Behaviour.SetOptionValue(DefaultValue);
        }

        private void SetUpscalingQuality(int quality)
        {
            if (!Options.CameraData.allowDynamicResolution)
                return;
            
            if (Options.CameraData.allowDeepLearningSuperSampling)
            {
                Options.CameraData.deepLearningSuperSamplingUseCustomQualitySettings = true;
                Options.CameraData.deepLearningSuperSamplingQuality = (uint)quality;
            }
            else if (Options.CameraData.allowFidelityFX2SuperResolution)
            {
                Options.CameraData.fidelityFX2SuperResolutionUseCustomQualitySettings = true;
                Options.CameraData.fidelityFX2SuperResolutionQuality = (uint)quality;
            }
        }
        
        public override void OnBuildOption(OptionBehaviour behaviour)
        {
            behaviour.SetOptionData(new StorableCollection() { { "options", UpscalingQualities } });
        }
    }
}